...
Plugin URI:
Description: My plugin to capture event details.
...
add_action( 'add_meta_boxes', 'add_your_fields2_meta_box' );
add_action( 'save_post', 'save_your_fields_meta' );
add_action( 'save_post', 'save_your_fields2_meta' );
...
function add_your_fields_meta_box() {
	...
	add_meta_box(
		'your_fields2_meta_box', // $id
		'Event Organisers', // $title
		'show_your_fields2_meta_box', // $callback
		'your_post', // $screen
		'normal', // $context
		'high' // $priority
	);
...
function show_your_fields_meta_box() {
    ...
    <!-- All fields with data retrieved will go here -->
		<p>
		...
		<p>
			<label for="your_fields[text]">Event Venues</label>
			<br>
			<input type="text" min="0" placeholder="Type a location" 
			name="your_fields[location]" id="your_fields[location]" 
			class="regular-text" value="<?php echo $meta['location']; ?>">
		</p>
<?php

}
function show_your_fields2_meta_box() {
	
	global $post;   
	$meta = get_post_meta( $post->ID, 'your_fields2', true );
	?>
	...
	<!-- All fields with data retrieved will go here -->
		<p>
			<label for="your_fields2[text]">Name / Company</label>
			<br>
			<input type="text" min="0" placeholder="Type a name" 
			name="your_fields2[name]" id="your_fields2[name]" 
			class="regular-text" value="<?php echo $meta['name']; ?>">
		</p>
...
//	5.2.1
//	hint: saves the data in the meta box for the second meta box
function save_your_fields2_meta( $post_id ) {   
	// verify nonce
	if ( !wp_verify_nonce( $_POST['your_meta_box_nonce'], basename(__FILE__) ) ) {
		return $post_id; 
	}
	...
	$old = get_post_meta( $post_id, 'your_fields2', true );
	$new = $_POST['your_fields2'];

	if ( $new && $new !== $old ) {
		update_post_meta( $post_id, 'your_fields2', $new );
	} elseif ( '' === $new && $old ) {
		delete_post_meta( $post_id, 'your_fields2', $old );
	}
}
